...
class Generator:
    def __init__(self, model: GeneratorModel, model_helper, max_attack_num: int, target_item: int, max_seq_len: int, log=None, device: str = 'cpu'):
        self.model = model
        ...
        for i in range(self.group_num):
            for j in range(self.max_attack_num):
                ...
                if is_adv:
                    ...
                    constraint_data = self.history_constraints[:, j, :, :].squeeze()
                    k = max(1, int(constraint_data.shape[0] * K_PERCENTILE))
