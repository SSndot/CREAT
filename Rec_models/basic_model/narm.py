...
class NARM(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        embedded_x, scores = self.model(x, self.embedding.token.weight, lengths, mask)
...
class NARMEmbedding(nn.Module):
    ...
    def get_lengths(seq):
        ...
        if seq.dim() == 1:
            return torch.count_nonzero(seq).type(torch.long)
        else:
            lengths = torch.count_nonzero(seq, dim=1).type(torch.long)
            lengths[lengths == 0] = 1
            return lengths
...
class NARMModel(nn.Module):
    ...
    def forward(self, x, embedding_weight, lengths, mask):
        ...
        b_out = self.b_vetor(embedding_weight).permute(1, 0)
        scores = torch.matmul(proj, b_out)
        return proj, scores
...
class NARMHelper(nn.Module):
    def __init__(self, model, args, device='cpu'):
        super().__init__()
        self.args = args
        self.model = model
        self.device = device
        self.model_name = 'narm'
    ...
    def forward(self, x, grads=False):
        ...
        embedded_x, scores = model(x, lengths)
        if not grads:
